...
// Phase anchor frequencies (analog-style spacing)
...
void RwSAllPass::onSetPins()
{
    ...
    // go silent ONLY if neither channel is streaming
...
{
    // never touch buffers unless at least one channel streams
    if (!pinInL_.isStreaming() && !pinInR_.isStreaming())
        return;

    const float* inL = getBuffer(pinInL_);
    const float* inR = pinInR_.isStreaming() ? getBuffer(pinInR_) : inL;
